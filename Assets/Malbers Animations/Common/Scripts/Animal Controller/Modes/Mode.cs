...
namespace MalbersAnimations.Controller
{
    ...
    public class Mode
    {
        ...
        {
            if (ResetToDefault && !Animal.IsOnZone) AbilityIndex = DefaultIndex; //Dont reset it if you are on a zone... the Zone will do it automatically if you exit it
        }
        ...
        public void AnimationTagExit(Ability exitingAbility)
        {
            ...
            {
                if (Animal.debugModes) Debug.Log($"Mode: <B>{Name}<color=red> [ANIMATION EXIT] </color> </B>  with Ability <b>[{(exitingAbility?.Name)}] </b>  Time: " + Time.time.ToString("F3"));
                ResetMode();
        ...
        public virtual bool StateCanInterrupt(StateID ID, Ability ability = null)
        {
            ...
            {
                if (properties.affect == AffectStates.Exclude && ContainState(properties,ID)      //If the new state is on the Exclude State
                || (properties.affect == AffectStates.Inlcude && !ContainState(properties,ID)))   //OR If the new state is not on the Include State
                {
