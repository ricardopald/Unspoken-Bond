...
namespace MalbersAnimations.Controller
{
    public partial class MAnimal
    {
        ...
        public virtual void ResetController()
        {
            ...
            {
                State StartState = State_Get(OverrideStartState);
                if (StartState != null && !StartState.IsActiveState)
                {
                    StartState.Activate();
        ...
        public void UpdateDamagerSet()
        {
            ...
            foreach (var at in Attack_Triggers) at.Owner = (transform);                 //Tell to avery Damager that this Animal is the Owner
        }
        ...
        {
            Vector3 move3 = new Vector3(inputAxis.x, inputAxis.y, 0);
            SetInputAxis(move3);
        }

        public virtual void SetInputAxisYZ(Vector2 inputAxis)
        {
            Vector3 move3 = new Vector3(0, inputAxis.x, inputAxis.y);
            SetInputAxis(move3);
        }
        ...
        {
            var MovementAxisNew = new Vector3(MovementAxis.x, upDown, MovementAxis.z);
            Target_Direction  = new Vector3(Target_Direction.x, upDown, Target_Direction.z);

            SetMovementAxis(MovementAxisNew);
        }
        ...
        public virtual void MoveDirection(Vector3 move)
        {
            ...
            DeltaAngle = MovementDetected ? Mathf.DeltaAngle(angleCurrent, targetAngle) : 0;
        }

        public virtual void RotateAtDirection(Vector3 Direction) => RotateAtDirection(Direction, 10);
        ...
        public virtual void RotateAtDirection(Vector3 direction, float angleTreshold)
        {
            ...
            if (debugGizmos) Debug.DrawRay(transform.position, Target_Direction, Color.black);
        }
        #endregion
        ...
        protected void TryActivateState()
        {
            ...
            foreach (var state in states)
            {
                ...
                if (state.OnActiveQueue && !state.OnQueue) //Wake UP the State that is no longer on QUEUE and it was activated! (PRIORITY FOR THE  QUEDED STATES)!
                {
                    state.ActivateQueued();
                    break;
                }

                if ((state.UniqueID + CurrentFrame) % state.TryLoop != 0) continue; //Check the Performance Loop 

                if (ActiveState.CanExit && state.Active && !ActiveState.IsPending &&
                    !state.IsSleepFromState && !state.IsSleepFromMode && !state.OnQueue
                    && state.TryActivate()) //Means a new state can be activated
                {
                    state.Activate();
                    break;
                }
                ...
                if (GoingDown) //if the Animal is going down but suddenly has bump into somehting RESET the GRAVITY VALYES UGLY FIX
                {
                    ...
                    if (deltaPosY < GravitySpeed && GravityTime > (m_gravityTime + 20)  && GravitySpeed > 1)
                    {
                       // Debug.Log("RESET Gravity");
                        ...
                        //ResetGravityValues();
                        //ResetUPVector();
                        InertiaPositionSpeed = Forward * ScaleFactor * DeltaTime * 3; //Force going forward
                        GravitySpeed = 0;
                        return;
                    }
